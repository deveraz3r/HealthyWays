...
class PharmacistDetailsPage extends StatefulWidget {
  static route(String pharmacistId) =>
      MaterialPageRoute(builder: (_) => PharmacistDetailsPage(pharmacistId: pharmacistId));
...
class _PharmacistDetailsPageState extends State<PharmacistDetailsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Pharmacist Details'), backgroundColor: AppPallete.backgroundColor2),
      body: Obx(() {
        ...
        if (_controller.selectedPharmacist.hasError) {
          return Center(child: Text('Error: ${_controller.selectedPharmacist.error?.message}'));
        }
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 16),
                      ListTile(leading: const Icon(Icons.email), title: Text(pharmacist.email)),
                      if (pharmacist.address != null)
                        ListTile(leading: const Icon(Icons.location_on), title: Text(pharmacist.address!)),
                    ],
                  ),
                ),
              ),
