...
abstract interface class DiaryRemoteDataSource {
  Future<List<DiaryModel>> getAllDiaryEntries(String patientId);
  Future<void> createDiaryEntry(DiaryModel diary);
  Future<void> deleteDiaryEntry(String id);
  Future<DiaryModel> updateDiaryEntry({
    required String id,
...
class DiaryRemoteDataSourceImpl implements DiaryRemoteDataSource {
  ...
  @override
  Future<List<DiaryModel>> getAllDiaryEntries(String patientId) async {
    try {
      ...
      final List<DiaryModel> diaryEntries =
          response.map((e) => DiaryModel.fromJson(e)).toList();
